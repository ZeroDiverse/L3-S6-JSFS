...
const GroupTable = ({groupNumber}) => {
    ...
    const setStudentToGroupNumber = async (student, groupNumber) => {
        try {
            const response = await axios.post(`/groups/${groupNumber}/students/${student._id}`)
            if(response.status === 201){
                setStudents(students.filter(s => s._id !== student._id))
                    setActionDescriptionText(`Student ${student.firstname} ${student.lastname} with student number: ${student.studentNumber} ASSIGNED to group ${groupNumber}`)
                setTimeout(() => {
    ...
    const unAssignStudentFromGroup = async (student) => {
        try {
            ...
            if (response.status === 200) {
                setStudents(students.filter(s => s._id !== student._id))
                setActionDescriptionText(`Student ${student.firstname} ${student.lastname} with student number: ${student.studentNumber} REMOVED from group ${groupNumber}`)
                setTimeout(() => {
                    setActionDescriptionText('')
                }, 2000)
            }
    ...
    useEffect(() => {
        if (groupNumber === 0) {
            ...
            axios.get(`/groups/${groupNumber}/students`)
                .then((res) => {
                    return res.data
                })
                .then(data => {
